...
import { onAuthStateChanged, signOut } from "firebase/auth"; // Added signOut for logout
      ...
      {name === "Profile" && !loadingAvatar ? (
        avatarUrl ? (
          <Image
            ...
            style={styles.avatar}
            onError={() => setAvatarUrl(null)} // Reset avatarUrl if loading fails
          />
        ) : (
          <IconComponent
            name={icon}
            size={24}
            color={activeTab === name ? "maroon" : "#aaa"}
          />
        )
...
const styles = StyleSheet.create({
  ...
  avatar: {
    width: 25,
    height: 25,
    borderRadius: 12, // Make the avatar circular
  },
});
